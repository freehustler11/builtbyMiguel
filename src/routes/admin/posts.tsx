      ...
      {posts.length === 0 ? (
        ...
        <div className="space-y-4">
          {posts.map((post) => {
            ...
            const isScheduledPast = post.status === 'scheduled' && post.scheduledAt && new Date(post.scheduledAt) <= new Date()
            const isPublished = post.status === 'published' || isScheduledPast
            const isScheduled = post.status === 'scheduled' && !isScheduledPast
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                      ...
                      />
                      <span className="capitalize">{isPublished ? 'Published' : post.status}</span>
                    </span>

                    {/* Category Badge */}
                    {post.category && (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
                    ...
                    {post.keyword && (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900/50">
                    ...
                    {hasCustomCta && (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-medium bg-cyan-50 dark:bg-cyan-950/50 text-cyan-600 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-900/50">
                  ...
                  <div className="flex items-center gap-2 self-start sm:self-auto">
                    ...
                    {isPublished ? (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-[6px] text-xs font-semibold text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/40 border border-amber-200 dark:border-amber-900 transition cursor-pointer"
                      >
                        Unpublish
                      </button>
                    ) : (
                      <button
                        ...
                        title={isScheduled ? 'Publish Live immediately ahead of schedule' : 'Publish Live immediately'}
                        className="px-3 py-1.5 rounded-[6px] text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900 transition cursor-pointer"
                      >
                        Publish Now
                      </button>
                    )}
                    ...
                    {isPublished && (
                      <Link
                        ...
                        target="_blank"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-[6px] text-xs font-semibold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 transition"
                      >
        ...
        onSelect={(chosen) => {
          if (mediaPickerTarget === 'cover') {
            ...
            if (chosen.mimeType.startsWith('image/')) {
              setInsertImageUrl(chosen.fileUrl)
              setInsertImageAlt(chosen.filename.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' '))
              setIsImageModalOpen(true)
            } else {
              insertMarkdown(`\n[📄 Download ${chosen.filename}](${chosen.fileUrl})\n`)
